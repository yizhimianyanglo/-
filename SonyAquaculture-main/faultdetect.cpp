...
{
    ui->XTQ->setRectType("大"); // 协调器
    ui->XTQ->setColorType("绿色");
    ui->autoSZJCmodule->setRectType("中"); // 自动水质检测模块
    ui->autoSZJCmodule->setColorType("绿色");
    ui->ZYKZmodule->setRectType("中"); //增氧控制模块
    ui->ZYKZmodule->setColorType("绿色");
    ui->DJLXJmodule->setColorType("绿色"); //电机螺旋桨模块
    ui->autoSWKZmodule->setRectType("中");//自动水温控制模块
    ui->autoSWKZmodule->setColorType("绿色");
    ui->autoSWKZmodule_2->setRectType("中");//自动水位控制模块
    ui->autoSWKZmodule_2->setColorType("绿色");
    ui->autoTWmodule->setRectType("中");// 自动投喂模块
    ui->autoTWmodule->setColorType("绿色");
    ui->KQHJJCmodule->setRectType("中");//空气环境监测模块
    ui->KQHJJCmodule->setColorType("绿色");
     ui->WDKZmodule->setColorType("绿色"); //温度控制模块
    ui->SDsensor1->setColorType("绿色"); //温度传感器1
    ui->SDsensor2->setColorType("绿色"); //温度传感器2
     ui->CSBsensor->setColorType("绿色"); //超声波传感器
    ui->SWsensor->setColorType("绿色");   // 水位传感器
    ui->JSSB->setColorType("绿色");//进水水泵
    ui->CSSB->setColorType("绿色"); //出水水泵
    ui->ZLsensor->setColorType("绿色"); //重力传感器
    ui->autoFM->setColorType("绿色"); //自动控制阀门
    ui->TWKZTmodule->setColorType("绿色"); //投喂控制台模块
...
void FaultDetect::faultDetect()
{
    //开始自检
    if (timerCount==1) {
        drawColor=QColor(18, 134, 190); // 连线的颜色
        ui->XTQ->setColorType("绿色");
        timerCount++;
    }
